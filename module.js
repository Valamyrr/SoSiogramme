...
const addPersoBtn = document.getElementById('addPersoBtn');
...
// ===== SYSTÈME DE MISE À JOUR DE RELATIONS =====
const updateModal = document.getElementById('updatePersoModal');
const updatePersoBtn = document.getElementById('updatePersoBtn');
const closeUpdateModalBtn = document.getElementById('closeUpdateModalBtn');
const addUpdateRelationBtn = document.getElementById('addUpdateRelationBtn');
const generateUpdateCodeBtn = document.getElementById('generateUpdateCodeBtn');
const updateRelationsContainer = document.getElementById('updateRelationsContainer');
const updatePersoSearch = document.getElementById('updatePersoSearch');
const updatePersoResults = document.getElementById('updatePersoResults');
const updatePersoSelect = document.getElementById('updatePersoSelect');

let relationCount = 0;
let updateRelationCount = 0;

// Ouvrir le modal d'ajout de personnage
addPersoBtn.addEventListener('click', () => {
...
updatePersoBtn.addEventListener('click', () => {
  updateModal.classList.add('active');
  document.getElementById('updateCodeOutput').classList.remove('active');
  resetUpdateForm();
});
...
function resetForm() {
  ...
  document.getElementById('persoImage').value = '';
  document.getElementById('persoGroupe').value = '';
  relationsContainer.innerHTML = '';
  relationCount = 0;
}

// Reset du formulaire de mise à jour
function resetUpdateForm() {
  updatePersoSearch.value = '';
  updatePersoSelect.value = '';
  updateRelationsContainer.innerHTML = '';
  updateRelationCount = 0;
  updatePersoResults.style.display = 'none';
}

// Recherche de personnage pour la mise à jour
updatePersoSearch.addEventListener('input', (e) => {
  ...
  const matches = nomsPersos.filter(p => 
    p.nom.toLowerCase().includes(searchTerm)
  );
  
  if (matches.length > 0) {
    updatePersoResults.innerHTML = matches.map(p => `
      <div class="search-result-item" 
           data-id="${p.id}" 
           data-nom="${p.nom}"
           style="padding: 10px; cursor: pointer; color: ${groupes[p.groupe]}; font-family: 'Merriweather'; font-size: 13px; border-bottom: 1px solid #2a2a2a; transition: 0.2s;">
        ${p.nom} <span style="opacity: 0.6; font-size: 11px;">(${p.groupe})</span>
      </div>
    `).join('');
    updatePersoResults.style.display = 'block';
    
    updatePersoResults.querySelectorAll('.search-result-item').forEach(item => {
      item.addEventListener('mouseenter', () => {
        item.style.background = 'rgba(194, 18, 8, 0.2)';
      });
      item.addEventListener('mouseleave', () => {
        item.style.background = 'transparent';
      });
      item.addEventListener('click', () => {
        updatePersoSelect.value = item.dataset.id;
        updatePersoSearch.value = item.dataset.nom;
        updatePersoResults.style.display = 'none';
      });
    });
  } else {
    updatePersoResults.innerHTML = '<div style="padding: 10px; color: #8999AC; font-family: Merriweather; font-size: 12px;">Aucun personnage trouvé</div>';
    updatePersoResults.style.display = 'block';
  }
});
...
// Ajouter une relation (modal d'ajout)
addRelationBtn.addEventListener('click', () => {
  relationCount++;
  addRelationField(relationsContainer, relationCount, 'relation');
});

// Ajouter une relation (modal de mise à jour)
addUpdateRelationBtn.addEventListener('click', () => {
  updateRelationCount++;
  addRelationField(updateRelationsContainer, updateRelationCount, 'update-relation');
});

// Fonction générique pour ajouter un champ de relation
function addRelationField(container, count, prefix) {
  const relationDiv = document.createElement('div');
  relationDiv.className = 'relation-item';
  relationDiv.dataset.relationId = `${prefix}-${count}`;
  
  const typeOptions = Object.keys(relationsColors).map(type => 
    `<option value="${type}">${getRelationLabel(type)}</option>`
  ).join('');
  
  relationDiv.innerHTML = `
    <div class="relation-header">
      <span>Relation #${count}</span>
      <button type="button" class="btn-remove-relation" onclick="removeRelation('${prefix}-${count}')">Supprimer</button>
    </div>
    <div style="position: relative;">
      <input type="text" 
             ...
             autocomplete="off"
             style="width: 100%; padding: 10px; background: rgba(13, 13, 13, 0.8); border: 1px solid #3a3a3a; color: #a79c99; font-family: 'Cinzel'; font-size: 14px; border-radius: 5px; box-sizing: border-box; margin-bottom: 10px;">
      <div class="search-results" style="display: none; position: absolute; top: 100%; left: 0; right: 0; background: rgba(13, 13, 13, 0.95); border: 1px solid #c21208; border-radius: 5px; max-height: 200px; overflow-y: auto; z-index: 1000; margin-top: -10px;"></div>
      <input type="hidden" class="relation-perso" value="">
    </div>
    <select class="relation-type-ab">
      <option value="">-- Mon perso → Son perso --</option>
      ${typeOptions}
    </select>
    <select class="relation-type-ba">
      <option value="">-- Son perso → Mon perso --</option>
      ${typeOptions}
    </select>
  `;
  ...
  const searchInput = relationDiv.querySelector('.relation-search');
  const searchResults = relationDiv.querySelector('.search-results');
  const hiddenInput = relationDiv.querySelector('.relation-perso');
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    if (matches.length > 0) {
      ...
      searchResults.innerHTML = '<div style="padding: 10px; color: #8999AC; font-family: Merriweather; font-size: 12px;">Aucun personnage trouvé</div>';
      searchResults.style.display = 'block';
    }
  });
  
  document.addEventListener('click', (e) => {
    if (!relationDiv.contains(e.target)) {
      searchResults.style.display = 'none';
    }
  });
}
...
function removeRelation(id) {
  const relationDiv = document.querySelector(`[data-relation-id="${id}"]`);
  if (relationDiv) {
    relationDiv.remove();
  }
}

// Générer le code (ajout de personnage)
generateCodeBtn.addEventListener('click', () => {
  const nom = document.getElementById('persoNom').value.trim();
  const image = document.getElementById('persoImage').value.trim();
  const groupe = document.getElementById('persoGroupe').value;
  
  // Validation
  if (!nom || !image || !groupe) {
    alert('⚠️ Veuillez remplir tous les champs obligatoires (nom, image, groupe)');
    return;
  }
  
  // Générer un ID unique basé sur le nom et le timestamp
  const timestamp = Date.now();
  const nomSafe = nom.toLowerCase().replace(/[^a-z0-9]/g, '');
  const persoId = `membre_${nomSafe}_${timestamp}`;
  
  // Code du personnage
  const codePerso = `{ id: "${persoId}", nom: "${nom}", image: "${image}", groupe: "${groupe}" },`;
  document.getElementById('codePerso').textContent = codePerso;
  
  // Code des relations
  const relations = [];
  document.querySelectorAll('#relationsContainer .relation-item').forEach(item => {
    const cible = item.querySelector('.relation-perso').value;
    const typeAB = item.querySelector('.relation-type-ab').value;
    const typeBA = item.querySelector('.relation-type-ba').value;
    
    if (cible && (typeAB || typeBA)) {
      let relationStr = `{ source: "${persoId}", cible: "${cible}"`;
      if (typeAB) relationStr += `, typeAversB: "${typeAB}"`;
      if (typeBA) relationStr += `, typeBversA: "${typeBA}"`;
      relationStr += ` },`;
      relations.push(relationStr);
    }
  });
  
  const codeRelationsBlock = document.getElementById('codeRelationsBlock');
  if (relations.length > 0) {
    document.getElementById('codeRelations').textContent = relations.join('\n');
    codeRelationsBlock.style.display = 'block';
  } else {
    codeRelationsBlock.style.display = 'none';
  }
  
  // Afficher la section de code
...
// Générer le code (mise à jour de relations)
generateUpdateCodeBtn.addEventListener('click', () => {
  const persoId = updatePersoSelect.value;
  
  // Validation
  if (!persoId) {
    alert('⚠️ Veuillez sélectionner votre personnage');
    return;
  }
  
  // Code des relations
  const relations = [];
  document.querySelectorAll('#updateRelationsContainer .relation-item').forEach(item => {
    const cible = item.querySelector('.relation-perso').value;
    const typeAB = item.querySelector('.relation-type-ab').value;
    const typeBA = item.querySelector('.relation-type-ba').value;
  ...
  if (relations.length === 0) {
    alert('⚠️ Veuillez ajouter au moins une relation');
    return;
  }
  
  document.getElementById('updateCodeRelations').textContent = relations.join('\n');
  
  // Afficher la section de code
  document.getElementById('updateCodeOutput').classList.add('active');
  
  // Scroll vers le code
  document.getElementById('updateCodeOutput').scrollIntoView({ behavior: 'smooth', block: 'nearest' });
});
